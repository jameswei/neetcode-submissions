class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        def dfs(heights: List[List[int]], coordinates: tuple[int, int], cells: Set, explored: Set):
            ...
            can_flow = (x-1 >= 0 and heights[x-1][y] >= cur_height) or (x+1 <= len(heights)-1 and heights[x+1][y] >= cur_height) or (y-1 >= 0 and heights[x][y-1] >= cur_height) or (y+1 <= len(heights[x])-1 and heights[x][y+1] >= cur_height)
            if not can_flow:
