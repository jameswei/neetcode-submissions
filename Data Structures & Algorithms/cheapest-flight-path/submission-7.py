class Solution:
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, k: int) -> int:
        ...
        # 优先级队列，(price, node, stops)，起点的stop数是0，
        ...
        while len(queue) > 0:
            ...
            # 飞到目的地了
            if airport == end:
                # 因为用的 price 优先级队列，所以到达 end 节点时一定就是成本最低的
                return price

            # 到此节点已经停留了 max_stops 次了，放弃此路径
            ...
            for (next_price, next_airport) in adj_list[airport]:
                ...
                if (price+next_price < cheapest_to[next_airport][stops+1]):
                    # 入队时就更新状态表，可以避免同样状态的节点重复入队
                    # 比如这个for-loop 中另一个相同(price+next_price, stops+1)
